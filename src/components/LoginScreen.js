...
const LoginScreen = ({ onLogin }) => {
  ...
  return (
    ...
    >
      ...
      {/* Lock Screen */}
      <div className={`absolute inset-0 flex flex-col justify-center items-center transition-all duration-700 ${showLogin ? 'opacity-0 translate-y-10 pointer-events-none' : 'opacity-100'}`}>
        {/* Time and Date */}
      ...
      {/* Login Screen */}
      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ${showLogin ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        {/* User Profile */}
        <div className="flex flex-col items-center space-y-6">
          ...
          <h1 className="text-white text-2xl font-light">{selectedUser.name}</h1>
                ...
                {users.map((user) => (
                  <button
                    ...
                    className={`flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${
                      selectedUser.id === user.id ? 'bg-white/10' : 'hover:bg-white/5'
                    }`}
                  >
