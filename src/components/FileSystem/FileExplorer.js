...
// src/components/FileSystem/FileExplorer.js
import React, { useState, useEffect } from "react";
import FileViewer from "./FileViewer";
import RightClickItem from "../RightClickItem";

const FileExplorer = ({ fileSystem = [], initialFolder, onOpenFile }) => {
  // Use initialFolder if provided; otherwise, default to first element in fileSystem if available.
  const [currentFolder, setCurrentFolder] = useState(
    initialFolder || (fileSystem.length > 0 ? fileSystem[0] : { name: "No Folder", children: [] })
  );
  ...
  // - If it is a PDF, call the parent's onOpenFile to open it in a new window.
  // - Otherwise, set it as the selected file to view its content inside FileExplorer.
  const openFile = (file) => {
  ...
  return (
    <div
      className="window flex h-full bg-white/80 rounded-lg shadow-lg overflow-hidden"
